...
export type AiTask = 'write' | 'curate' | 'translate';
...
class OllamaProvider implements LlmProvider {
  ...
  constructor(private url: string, private model: string, private task: AiTask = 'write') {}
...
class DeepSeekProvider implements LlmProvider {
  ...
  constructor(private url: string, private apiKey: string, private model: string, private task: AiTask = 'write') {
...
class AnthropicProvider implements LlmProvider {
  ...
  constructor(private apiKey: string, private model: string, private task: AiTask = 'write') {
...
export async function providerForTask(task: AiTask): Promise<LlmProvider> {
  const provider = (await getConfig(`ai.${task}.provider` as 'ai.write.provider')) as ProviderName;
  const model = await getConfig(`ai.${task}.model` as 'ai.write.model');
